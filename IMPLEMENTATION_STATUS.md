...
The longer GitOps bring-up exposed two memory limits that startup checks missed: repo-server OOM during concurrent chart rendering, and Tempo OOM during block completion. The local bootstrap repo-server correction passed a hard-refresh retest. The isolated Tempo regression now passes completed-block restart plus SIGKILL/replay of a verified non-empty 5,000-trace WAL under the proposed 768Mi limit. A separate Kubernetes regression reproduces an unready OrderedReady rollout and verifies Pod-only recovery preserves the PVC UID and data. The follow-up changes require review; **live Git-managed Tempo remains unhealthy until the reviewed fix is applied and rechecked**. Do not treat these isolated checks or the earlier correlation success as current full-stack health. See the [GitOps adoption evidence](docs/evidence/2026-09-05-gitops.md).
