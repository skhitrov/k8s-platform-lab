...
The [public repository](https://github.com/skhitrov/k8s-platform-lab) is pushed; hosted CI and the verified multi-architecture GHCR release succeeded. [Dev PR #1](https://github.com/skhitrov/k8s-platform-lab/pull/1) awaits human review. GitOps adoption and reviewed promotion are the next delivery gates, not silently bypassed implementation steps. Branch-protection configuration is supplied in `.github/branch-protection.json`; inspect GitHub for its effective live settings. No cloud provider or paid resource is involved. `k3s-lab` is running with the tested add-ons; Kind is stopped with data preserved; the default profile remains untouched. Exact dependency pins are in `platform/versions.yaml`, not the older list below.
